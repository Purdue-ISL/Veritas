...
def test_annotate(*, kind: str, directory: str, initial: str, transition: str, emission: str) -> None:
    R"""
    Test HMM model annotations.
    ...
    framework.phase2(ARGS[kind](directory, initial, transition, emission))
    framework.erase()

    #
    framework._model.__annotate__()
    framework._model.initial.__annotate__()
    framework._model.transition.__annotate__()
    framework._model.emission.__annotate__()
...
def main() -> None:
    ...
    for (kind, directory, initial, transition, emission) in PARAMETERS:
        #
        test_annotate(kind=kind, directory=directory, initial=initial, transition=transition, emission=emission)
