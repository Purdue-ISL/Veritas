...
#
PARAMETERS = [
    ("categorical", DATA_DIR0, "generic", "generic", ""),
    ("categorical", DATA_DIR1, "", "generic", "categorical"),
    ("categorical", DATA_DIR1, "generic", "", "categorical"),
    ("gaussian", DATA_DIR2, "generic", "generic", ""),
    ("gaussian", DATA_DIR3, "", "generic", "gaussian"),
    ("gaussian", DATA_DIR3, "generic", "", "gaussian"),
    ("stream", DATA_DIR4, "generic", "generic", ""),
    ("stream", DATA_DIR4, "", "generic", "v0"),
    ("stream", DATA_DIR4, "generic", "", "v0"),
]
...
def test_hmm(*, kind: str, directory: str, initial: str, transition: str, emission: str) -> None:
    R"""
    Test unknown HMM model.
    ...
    framework.phase1(ARGS[kind](directory, initial, transition, emission))
    framework.erase()

    # Call this will raise error, so it must be after previous phase and erasion.
    framework.models()
...
def main() -> None:
    ...
    for (kind, directory, initial, transition, emission) in PARAMETERS:
        ...
        try:
            #
            test_hmm(kind=kind, directory=directory, initial=initial, transition=transition, emission=emission)
        except RuntimeError:
            #
            pass
