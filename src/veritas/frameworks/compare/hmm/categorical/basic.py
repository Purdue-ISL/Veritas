...
class FrameworkCompareHMMCategoricalBasic(
    FrameworkCompareHMMCategorical[AlgorithmGradientHMM, AlgorithmConventionHMM],
):
    R"""
    Framework to compare two algorithms on categorical HMM dataset.
    Two algorithms are:
    1. PyTorch categorical HMM (testing);
    2. HMMLearn categorical HMM (baseline).
    """

    def models(self: FrameworkCompareHMMCategoricalBasic, /) -> None:
        R"""
        Prepare model(s).
        ...
        self._model_base = CategoricalHMM(
        ...
        (emissions,) = self._model_test.emission.parameters()
        ...
        self._model_base.emissionprob_ = onp.zeros_like(emissions.data.cpu().numpy())

    def algorithms(self: FrameworkCompareHMMCategoricalBasic, /) -> None:
        R"""
        Prepare algorithm(s).
    ...
    def communicate(self: FrameworkCompareHMMCategoricalBasic, /) -> None:
        R"""
        ...
        (emissions,) = self._algorithm_test.model.emission.parameters()
        ...
        onp.copyto(self._algorithm_base.model.emissionprob_, emissions.data.cpu().numpy())

    def validate_loss(self: FrameworkCompareHMMCategoricalBasic, /) -> None:
        R"""
    ...
    def validate_metric(self: FrameworkCompareHMMCategoricalBasic, /) -> None:
        R"""
    ...
    def validate_parameter(self: FrameworkCompareHMMCategoricalBasic, /) -> None:
        R"""
        ...
        assert self.equal(
            ...
            self._algorithm_base.model.emissionprob_,
        ), "Emission parameter does not match."

    def validate_final(self: FrameworkCompareHMMCategoricalBasic, /) -> None:
        R"""
